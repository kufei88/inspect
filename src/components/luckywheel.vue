<template>
    <div class="container">
        ...
        <div class="main">
            ...
            <div class="bg-p"></div>
            <div class="content">
                <div class="lottery_ticket">你还有{{wheelCount}}次抽奖机会。(本次抽奖时间：{{wheel.beginDate}}--{{wheel.endDate}})</div>
            </div>
            <div class="tip">
                <div class="tip-title">活动规则</div>
                <div class="tip-content">
                    <p>  </p>
                    <p> 1.一等奖：{{wheel.firstName}}</p>
                    <p> 2.二等奖：{{wheel.secondName}}</p>
                    <p> 3.三等奖：{{wheel.thirdName}}</p>
                </div>
            </div>
        </div>
...
export default {
  data() {
    return {
      ...
      prize_list: [
        {
          icon: require("@/assets/img/一等奖.png"), // 奖品图片
          count: 10, // 奖品数量
          name: "一等奖", // 奖品名称
          isPrize: 1 // 该奖项是否为奖品
        },
        {
          icon: require("@/assets/img/二等奖.png"),
          count: 5,
          name: "二等奖",
          isPrize: 1
        },
        ...
        {
          icon: require("@/assets/img/三等奖.png"),
          count: 5,
          name: "三等奖",
          isPrize: 1
        },
  ...
  methods: {
    ...
    init_prize_list(list) {
      let _this = this;
      HttpUtil.get("wheel/getActivityWheels.do", {}, function(data) {
        _this.wheel = data;
        if (data) {
        }
        _this.prizeNameList.push(_this.wheel.firstName);
        _this.prizeNameList.push(_this.wheel.secondName);
        _this.prizeNameList.push(_this.wheel.thirdName);
      });
      HttpUtil.get(
        "wheel/getWheelCount.do",
        { client: this.$store.state.vux.snsUserInfo.username },
        function(data) {
          _this.wheelCount = data;
        }
      );
    },
    rotate_handle() {
      ...
      if (
        this.wheel.firstCount + this.wheel.secondCount + this.wheel.thirdCount <
        1
      ) {
        this.$vux.alert.show({
          title: "提示",
          content: "奖品已经抽光了，下次早点"
        });
        return;
      }
      console.log(this.wheel.firstCount);
      console.log(this.wheel.secondCount);
      console.log(this.wheel.thirdCount);
      let random = Math.floor(
        Math.random() *
          (this.wheel.firstCount +
            this.wheel.secondCount +
            this.wheel.thirdCount)
      ); //指定每次旋转到的奖品下标
      if (random < this.wheel.firstCount) {
        this.index = 0;
        this.wheel.firstCount -= 1;
      } else if (random < this.wheel.firstCount + this.wheel.secondCount) {
        this.index = 1;
        this.wheel.secondCount -= 1;
      } else {
        this.index = 3;
        this.wheel.thirdCount -= 1;
      }
      console.log(this.wheel.firstCount);
      console.log(this.wheel.secondCount);
      console.log(this.wheel.thirdCount);
    ...
    game_over() {
      ...
      this.prize = this.prize_list[this.index].name;
      let data = {
        client: this.$store.state.vux.snsUserInfo.username,
        wheelId: this.wheel.id,
        prize: this.prize_list[this.index].name
      };
      this.toast_content = this.prizeNameList[this.index];
      let _this = this;
      HttpUtil.post(
        "wheel/insertWheelRecord.do",
        { jsonStr: JSON.stringify(data) },
        function(data) {
          _this.wheelCount = _this.wheelCount - 1;
        }
      );
    },
    //关闭弹窗

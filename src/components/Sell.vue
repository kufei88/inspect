<template>
  <div class="order" style="padding-bottom: 50px">
    ...
    <group :title="'收货信息:'+beginDate+'--'+endDate" class="main">
      <sell-list :title="title"  :list="billlist" :showMenu="false" 　@on-del-item="delBill($event)"
       @on-edt-item="edtBill($event)" ></sell-list>
...
export default {
  ...
  methods:{
    ...
    getBillList(){
      ...
      HttpUtil.get('bill/getSellByClient.do',{client:this.$store.state.vux.snsUserInfo.username,
        beginDate:searchDate,endDate:searchEndDate},
      function(data){
        data.forEach(function(item){
          if ((billcode !='') && (billcode != item.goodCode)) {
            templist.push({billdate:billdate,billcode:billcode,goodslist:goodlist,audite:audite,sum:sum});
            goodlist = [];
            sum = 0;
          }
          ...
          goodlist.push({goodsName:item.stockGoodsName,number:item.stockGoodsNum,price:item.stockGoodsPrice});
        })
        templist.push({billdate:billdate,billcode:billcode,goodslist:goodlist,audite:audite,sum:sum});
